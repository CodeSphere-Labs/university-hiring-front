import { Button, Flex, Group, Input, Loader, Select, Stack, TextInput } from '@mantine/core';
...
export const InviteUser = () => {
  const [organizations, groups, organizationsLoading, groupsLoading, user] = useUnit([
    $organizations,
    $groups,
      ...
      >
        ...
        <Group w={{ base: '100%', xs: '25%' }}>
          <Input.Wrapper label='Нет подходящей организации?'>
            <Button fullWidth mt={10} variant='outline' onClick={() => modalOpened()}>
              Создать
            </Button>
          </Input.Wrapper>
        </Group>
      ...
      {fields.role.value === 'STUDENT' && (
        ...
        >
          <Select
            ...
            onChange={(value) => {
              const selectedGroup = groups.find((group) => group.id.toString() === value);
          ...
          <Group w={{ base: '100%', xs: '25%' }}>
            <Input.Wrapper label='Нет подходящей группы?'>
              <Button fullWidth mt={10} variant='outline' onClick={() => createGroupModalOpened()}>
                Создать
              </Button>
            </Input.Wrapper>
          </Group>
